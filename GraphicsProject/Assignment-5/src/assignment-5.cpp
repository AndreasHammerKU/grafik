...
bool Flatness(BezierRow const& G, float epsilon)
{
   ...
   float l = glm::length(G1G4);
   float d1 = glm::length(G1G2 - glm::dot(G1G2, u)* u); 
   float d2 = glm::length(G4G3 - glm::dot(G4G3, u)* u); 
   if (l < epsilon){
     return false;
   }
   if (glm::dot(G1G2, u) < 0) {
